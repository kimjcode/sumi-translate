...
首次啟動會自動跳出設定視窗引導你完成。之後 Sumi 平常隱形，需要時從選單列的墨印 →「設定…」再打開。

### 1. 授予「輔助使用」權限

Sumi 靠雙擊 `⌘C` 觸發，這需要 macOS 的**輔助使用（Accessibility）**權限。點設定裡的「啟用權限」按鈕，在跳出的系統視窗授權即可，授權後自動開始運作、不需重啟（若 10 秒內沒反應，重啟一次）。

> Sumi 只偵測觸發鍵，**不記錄你打的任何內容**。

> **權限授權後失效？**（未簽章版的已知狀況）
> 目前提供的是未經 Apple 簽章的版本，每次重新打包後，先前的「輔助使用」授權會失效（清單顯示開著、卻沒作用）。解法：到「輔助使用」清單把 **Sumi 整條移除（−），再重新授權**（只切換開關沒用）。設定頁有「重新檢查」鈕協助你確認。
> 為什麼暫不簽章：見 [docs/decisions.md](docs/decisions.md)。

### 2. 貼上 API key

到「設定 → 翻譯引擎」，貼上 **Google Cloud Translation** 的 API key（或改用 DeepL）。進階的 AI 字義功能可另外貼 **Gemini** key（只在字典查無某字時用；字典本身免 key、離線運作）。

> **還沒有 key？** 申請步驟、費用與安全設定見 [docs/setup-google-api.md](docs/setup-google-api.md)。
> key 只存於 macOS Keychain，不寫進任何檔案。

### 3. 選擇語言

預設是「語言配對 繁中 ⇄ English」（自動判斷翻譯方向）。可改對照語言，或切換成「固定目標語言」。

---

## 給開發者：從原始碼執行時的注意事項

> 以下只在你**自己 build／用 `npm run tauri dev` 跑**時相關；直接使用打包好的 App 可略過。

- **輔助使用權限**要授予給「啟動指令的那個 App」——dev 模式下通常是你的終端機（Terminal／iTerm／VS Code），不是 Sumi 本身。
- **每次啟動後第一次翻譯，可能跳一兩次 Keychain 密碼框**：因為 `tauri dev` 的 binary 未簽章、每次重編都不同，Keychain 認不得它。打包成正式 App 後此提示消失。
...
- 申請 Google API key：[docs/setup-google-api.md](docs/setup-google-api.md)
