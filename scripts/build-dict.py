...
def build(src_db, cc):
    ...
    rows = src.execute(
        "SELECT word, phonetic, pos, translation, bnc, frq, tag, collins, oxford FROM stardict"
    )
    batch, n, skipped = [], 0, 0
    for word, phonetic, pos, translation, bnc, frq, tag, collins, oxford in rows:
    ...
    print(f"\n完成：收錄 {n} 單字（濾掉 {skipped} 片語/非單字）→ {OUT}（{size_mb}MB）")
