...
## D12 — 移除「always-on 剪貼簿監聽」死功能，定位純雙擊觸發（2026-06-23）

設定頁「進階」section 原有「always-on 剪貼簿監聽（複製即翻）即將推出」預告。經查證是**死功能**：UI 假開關（寫死 `checked={false} disabled`、未綁任何 state）＋ 後端 `always_on_monitor: bool` 死欄位（全專案零消費者，`monitor/` 只跑雙擊 ⌘C）＋ PRD §3.3/§4.1 的「選用模式」承諾。**沒有任何真正的剪貼簿監聽實作。**

- **決策**：三層**全部移除，不保留、不降級到 backlog**。
- **理由**：① 「即將推出」是對**還沒決定要做**的功能的公開承諾（已開源）；② always-on 自動監聽與 Sumi 核心定位**直接衝突**——Sumi 是「雙擊 ⌘C 才觸發、平常不監聽、隱私優先、克制」，留著承諾會讓定位自相矛盾；③ 死欄位/假開關是半成品痕跡，日後只會讓人困惑。
- **移除範圍**：`SettingsView.tsx` 整個「進階」section；`settings.rs` 的 `always_on_monitor`（struct 欄位＋default）；`api.ts` 型別；`PRD.md` §3.3/§4.1 兩處承諾（§3.3「監聽 vs 雙擊」改寫為「採雙擊觸發、平常不監聽、不做 always-on」）。
- **相容性**：`Settings` 用 `#[serde(default)]`，舊 `settings.json` 殘留 `always_on_monitor` key 會被 serde 忽略、不報錯。
- **定位收斂**：Sumi 不做 always-on，沒有模糊地帶。日後若真要重議，需重新拍板（不是「恢復一個既有開關」，而是新功能提案）。

## D11 — 多行夾帶機密整段送 MT：D3 取捨下的已知可接受風險，不收緊（2026-06-18）

對應稽核 [audit-20260618.md M4](audit-20260618.md)。**維持現狀、不收緊**，記此一筆避免日後重議。

- **情境**：MT（快翻）路徑下，`looks_like_secret`（`monitor/filter.rs:48-72`）對多行只殺 PEM/PGP 區塊；`KEY=value` 賦值規則在 `s.lines().count() > 1` 時 `return false`。所以貼整份 `.env`／多行 log（含 `api_key=sk-...` 那行）會被判 `Text`、**整段送 Google/DeepL**（含密碼那行）。
- **為何不是 bug**：這是 [D3](decisions.md)「log／程式碼照翻、寧可放行」的**刻意取捨**——多行內容的核心使用情境就是翻 log／程式碼，逐行殺會大量誤殺正常內容。
- **與紅線的關係**：紅線「疑似機密不送外部」在**單行**賦值與已知 token 樣式上仍生效；多行的張力屬 D3 已接受範圍。注意這只涉及 **MT 翻譯路徑**；Workbench 字典路徑（`gemini_define`）的機密破口已於第一批 H2 修掉（送「該字所在句」並過濾，[issues #15](issues.md)）。
- **決策**：**已知、可接受，不收緊**。維持「多行除 PEM/PGP 外照翻」。
- **若未來要收緊**（目前不做）：多行時改逐行掃 `secret_assignment_value` 與已知前綴 token，命中就**遮該行**再送（而非整段放行或整段殺）。需另行拍板再做。
